...
class Chat {
  ...
  base() {
    ...
    this.router.text(/\/addgroup (\w+) (-?\d+)/, async (req, res) => {
      // Parse parameters
      const match = req.message.text.match(/\/addgroup (\w+) (-?\d+)/);
      if (!match) {
        await this.main.bot.sendMessage(req.chatId, '❌ Usage: /addgroup <groupname> <telegram_channel_id>');
        return;
      }
      ...
      try {
        // Use your new DB public methods!
        const exists = await this.main.db.groupExists(groupName);
        if (exists) {
          await this.main.bot.sendMessage(req.chatId, `❗️ Group "${groupName}" already exists.`);
          return;
        }
        await this.main.db.addGroup(groupName, channelId);
        await this.main.bot.sendMessage(req.chatId, `✅ Group "${groupName}" added and linked to channel ${channelId}.`);
      } catch (e: any) {
        const errMsg = (e instanceof Error) ? e.message : String(e);
        await this.main.bot.sendMessage(req.chatId, `❌ Error: ${errMsg}`);
      }
    });

    // /listgroups handler: show groups with only remove buttons (by channel ID)
    this.router.textOrCallbackQuery(/\/listgroups/, async (req, res) => {
      try {
        const groups = await this.main.db.listGroups();
        if (!groups.length) {
          await this.main.bot.sendMessage(req.chatId, 'No groups found.');
          return;
        }
        const lines = groups.map(g => `• ${g.name} — <code>${g.telegram_channel_id}</code>`);
        const keyboard = groups.map(g => [
          // Only Remove button, no Edit button
          { text: "🗑 Remove", callback_data: `/removegroupid/${g.telegram_channel_id}` }
        ]);
    ...
    // Remove group - ask for confirmation (by channel ID)
    this.router.callback_query(/\/removegroupid\/(.+)/, async (req, res) => {
      // Robustly extract channelId for callback, fallback to regex if not present
      let channelId = req.params && req.params[0];
      if (!channelId && req.callback_query && req.callback_query.data) {
        const m = req.callback_query.data.match(/\/removegroupid\/(.+)/);
        channelId = (m && m[1]) || '';
      }
      if (!channelId) {
        await this.main.bot.sendMessage(req.chatId, `❌ Error: Channel ID is missing for removal!`);
        return;
      }
      try {
        const group = await this.main.db.getGroupByChannelId(channelId);
        const groupName = group && group.name ? group.name : "Unknown Group";
        if (!group) {
          await this.main.bot.sendMessage(req.chatId, `Group not found!`);
          return;
        }
        await this.main.bot.sendMessage(req.chatId, `Are you sure you want to remove group "${groupName}"?`, {
          reply_markup: {
            inline_keyboard: [
              [
                { text: "✅ Yes, remove", callback_data: `/confirmremovegroupid/${channelId}` },
                { text: "❌ Cancel", callback_data: '/listgroups' }
              ]
            ]
          }
        });
    ...
    // Confirm and remove group (by channel ID)
    this.router.callback_query(/\/confirmremovegroupid\/(.+)/, async (req, res) => {
      // Robustly extract channelId for callback, fallback to regex if not present
      let channelId = req.params && req.params[0];
      if (!channelId && req.callback_query && req.callback_query.data) {
        const m = req.callback_query.data.match(/\/confirmremovegroupid\/(.+)/);
        channelId = (m && m[1]) || '';
      }
      if (!channelId) {
        await this.main.bot.sendMessage(req.chatId, `Failed to remove group: channel ID is missing!`);
        return;
      }
      try {
        const group = await this.main.db.getGroupByChannelId(channelId);
        ...
        await this.main.db.removeGroupByChannelId(channelId); // Only pass channelId
        await this.main.bot.sendMessage(req.chatId, `Group "${groupName}" removed!`);
      } catch (e) {
        const errMsg = (e instanceof Error) ? e.message : String(e);
        await this.main.bot.sendMessage(req.chatId, `Failed to remove group: ${errMsg}`);
      }
    });

    // Edit group UI and logic removed (users can only remove groups now)
  ...
  if (page > 0) {
    ...
    menu = [
      [
        ...
        {
          text: 'Show group list',   // <-- NEW BUTTON
          callback_data: '/listgroups'
        }
      ],
      [
        {
